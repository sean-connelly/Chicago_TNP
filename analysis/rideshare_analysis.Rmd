...
library(leaflet.extras);library(tmap);library(DT)
library(gghighlight);library(plotly);library(knitr)
...
fares_pct <- fares %>%
  ...
  mutate("trips_pct" = trips / sum(trips),
         "trips_pct_cuml" = cumsum(trips_pct)) %>% 
  ungroup()

# 95% of trips in all periods <$30 dollars
fares_tail <- fares_pct %>% 
  group_by(covid_group, year) %>% 
  filter(trips_pct_cuml < 0.95) %>% 
  arrange(desc(fare)) %>% 
  slice(1) %>% 
  ungroup()
...
```{r origin-destination tracts data import}
