...
tmap_od_flows_ccas <- 
  ...
  tm_shape(essential_workers_sf %>% 
           ...
           name = "Share of Chicago Essential Workers<br>Top 20 Percent of Tracts") +
  tm_borders(col = "black", lwd = 0.75) +
  tm_layout(main.title = "Rideshare Trips by Origin-Destination (OD)<br>Community Areas", 
...
```{r od flows executive summary}

# Executive summary OD flows
tmap_od_flows_exec_summ <- 
  tm_basemap(leaflet::providers$CartoDB.Positron) +
  tm_shape(sf_ccas, name = "Community Areas") +
  tm_borders(lwd = 0.5) +
  tm_shape(od_pairs_ccas_comp %>% 
           ...
           name = "OD Flows") +
  tm_lines(col = "trips", title = "Rideshare Trips After<br>Stay-at-Home Order (3/21/20)<br>by Origin-Destination<br>Community Areas", 
           style = "log10_pretty", palette = "plasma",
           lwd = "lwd", title.lwd = "Trips",
           scale = 10, alpha = 0.5,
           legend.lwd.show = FALSE) +
  ...
  #                              na.rm = TRUE)),
  #          legend.show = TRUE,
  #          name = "Share of Chicago<br>Essential Workers<br>Top 20 Percent of Tracts") +
  # tm_borders(col = "black", lwd = 0.75) +
...
gg_monthly_drivers_active <- monthly_drivers %>%
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  scale_y_continuous(labels = comma) +
  labs(title = "Active Drivers per Month declined starting in March 2020, bottoming out at 17,000 in April.\nThe number of active drivers remained below 50% of 2019 levels.",
...
**Rideshare trips decreased by 62 million (55%) in 2020 compared to 2019.** Prior to the Stay-at-Home Order going into effect (March 21, 2020), trips were already down by about 7% year-over-year. However, the bulk of the decline in rideshare occurred in the following 9 months. From late March 2020 onwards, only 27 million rideshare trips were taken in Chicago, about 60 million fewer trips compared to the same period in 2019.

**Weekday trips shifted to the afternoon, and average trip duration fell by 3 minutes (-15%), but average trip distance and fare remained unchanged.** The twin peaks of the morning and evening rush during weekdays flattened out after the Stay-at-Home Order went into effect compared to 2019. Average trip duration decreased by almost 3 minutes, likely due to the decrease in congestion. The average trip distance and fare were relatively unaffected. From late March 2020 onwards, the average trip distance increased to only slightly to 6.5 miles, a 1.7% increase compared to the same period in 2019. The average trip fare during this time was about the same as in 2019.

**Trip origins and destinations were mostly unchanged. Excluding airport trips (6%-9% of all trips), the most common rides were short trips to and from the Loop and nearby areas. In both 2019 and 2020, the top 3 Community Areas (The Loop, Near North Side, and Near West Side) accounted for over 40% of all rideshare trips within the city.** There is also a clear pattern of trips up and down the lakefront, particularly on the North Side. After the Stay-at-Home Order went into effect, trips linking the South lakefront to the Loop and the North Side decreased dramatically.

**While few rideshare trips were made on the South and West Sides compared to other parts of the city, rideshare trip behavior was less affected during the pandemic.** Neighborhoods on the South and West Sides saw a relative increase in their percentile rank, indicating an inelastic demand for rideshare. For example, one Census Tract in the West Side neighborhood of Austin was middle of the pack for trip pickups in 2019 (56th percentile). Pickups were almost cut in half in 2020, but the tract qualified as one of the more active pickup spots in the city (75th percentile). These South and West Side neighborhoods include some areas with a high concentration of essential workers, but the parts of the Southwest and Northwest Sides that saw the largest relative dropoff in their trip ranking are also home to many essential workers.

**The number of active drivers completing 1+ trip per month decreased by 45,000 (-64%) after the Stay-at-Home Order went into effect.** This trend was also paired by a large increase in the number of inactive drivers, defined as those who completed zero trips despite being eligible to drive in Chicago that month. During this period, a higher percentage of drivers were inactive (64%) compared to 2019 (34%). A higher percentage of trips (+7%) were made by drivers with over a year's experience, both before and after the Stay-at-Home Order went into effect. Given that this trend was present during the early months of 2020 as well, it is unlikely that this was driven by the pandemic, but the shift is notable. Before and after the pandemic took hold, almost all trips were made by drivers residing in Chicago (57%) or somewhere else in the State of Illinois (37%). Very few trips were made by drivers from other states.

</div>

<div class="exec-summ-column-right">

```{r executive summary plots}
